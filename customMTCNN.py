...
class CustomMTCNN(nn.Module):
    ...
    def create_image_pyramid(self, img: torch.Tensor) -> List[torch.Tensor]:
        ...
        return [F.interpolate(img, scale_factor=s, mode='bilinear', align_corners=True)
                for s in scales]
    
    def generate_bounding_boxes(self, det: torch.Tensor, box: torch.Tensor,
                              scale: float, threshold: float) -> np.ndarray:
